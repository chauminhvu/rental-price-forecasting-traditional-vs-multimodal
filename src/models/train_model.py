...
from define_model import (RentalDataModule, MLPRegressor,
                          Swish, LearnedSnake, load_mlp_params)
...
if RESUME:
    ...
    mlp_model.hparams['learning_rate'] = 1e-3
...
mlp_rmse_test = np.sqrt(mean_squared_error(y_hat_test, y.detach().cpu().numpy()))
print(f"R2 score: {mlp_r2_test}")
print(f"RMSE    : {mlp_rmse_test}")
...
# xgb_rmse_test = np.sqrt(mean_squared_error(xgb_pred_test, datamodule.y_test_scaled))
# print(f"XGBRegressor - R2 score  : {xgb_r2_test}")
# print(f"XGBRegressor - RMSE score: {xgb_rmse_test}")
